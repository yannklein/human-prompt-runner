#!/usr/bin/env python3
"""
Ecosystem Prompt Runner — ChatGPT

Runs ecosystem prompts (P1-P9) that don't require company-specific information.

Usage:
    python ecosystem_runner_chatgpt.py
    python ecosystem_runner_chatgpt.py --test
    python ecosystem_runner_chatgpt.py --headless --delay 10
"""
...
from runners.company.company_runner_chatgpt import PromptRunner
...
async def main():
    parser = argparse.ArgumentParser(description="Run ecosystem prompts on ChatGPT")
    ...
    if args.resume:
        ...
        run_folder_name = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S") + "_chatgpt_ecosystem"
        run_folder = results_root / run_folder_name
